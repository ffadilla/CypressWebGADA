...
export default class CreatePurchaseRequestPage extends BasePage {
  ...
  expandChannelList() {
    cy.get(this.CHANNEL_INPUT).click();
  }
  ...
  typeVendorId(vendorId) {
    cy.get(this.VENDOR_ID_INPUT).type(vendorId);
  }

  typeBuyerId(buyerId) {
    cy.get(this.BUYER_ID_INPUT).type(buyerId);
  }
  ...
  selectDeliveryMethod(deliveryMethod) {
    cy.get(this.DELIVERY_METHOD_INPUT + `[value='${deliveryMethod}']`).click();
  }

  setDeliveryRequestStartDate(date) {
    cy.get(this.DELIVERY_REQUEST_DATE_BUTTON).eq(0).click();

    cy.get(`button[aria-label='${date}']`).eq(0).click();
  }

  setDeliveryRequestEndDate(date) {
    cy.get(this.DELIVERY_REQUEST_DATE_BUTTON).eq(1).click();
    ...
    cy.get("button").then(($button) => {
      if ($button.find(`[aria-label='${date}']`).length > 1) {
        ...
        cy.get(this.NEXT_MONTH_CALENDAR_BUTTON).then((element) => {
          if (element.length > 1) {
            cy.get(this.NEXT_MONTH_CALENDAR_BUTTON).eq(1).click();
          } else {
            cy.get(this.NEXT_MONTH_CALENDAR_BUTTON).eq(0).click();
          }
        });
        cy.get(`button[aria-label='${date}']`).click();
      }
  ...
  typeDeliveryFee(deliveryFee) {
    cy.get(this.DELIVERY_FEE_INPUT).type(deliveryFee);
  }

  typeDeliveryFeeDiscount(deliveryFeeDiscount) {
    cy.get(this.DELIVERY_FEE_DISCOUNT_INPUT).type(deliveryFeeDiscount);
  }

  typeUnloadingFee(unloadingFee) {
    cy.get(this.UNLOADING_FEE_INPUT).type(unloadingFee);
  }

  typePurchaseDiscount(purchaseDiscount) {
    cy.get(this.PURCHASE_DISCOUNT_INPUT).type(purchaseDiscount);
  }

  typePurchaseReason(purchaseReason) {
    cy.get(this.PURCHASE_REASON_INPUT).type(purchaseReason);
  }

  clickNextToAddItem() {
    cy.get(this.NEXT_TO_ADD_ITEM_BUTTON).click();
  }

  clickAddNewItem() {
    cy.get(this.ADD_NEW_ITEM_BUTTON).click();
  }

  typeProduct(product) {
    cy.get(this.PRODUCT_INPUT).type(product);
  }

  selectTaxType(taxType) {
    cy.get(this.TAX_TYPE_INPUT + `[value='${taxType}']`).click();
  }

  typeQuantity(quantity) {
    cy.get(this.QUANTITY_INPUT).eq(0).type(quantity);
  }

  typeRate(rate) {
    cy.get(this.RATE_INPUT).eq(1).type(rate);
  }

  typeRateDiscount(rateDiscount) {
    cy.get(this.RATE_DISCOUNT_INPUT).type(rateDiscount);
  }

  clickAddItem() {
    cy.get(this.ADD_ITEM_BUTTON).click();
  }

  clickNextToSuggestedSellingPrice() {
    cy.get(this.NEXT_TO_SELLING_PRICE_BUTTON).click();
  }

  typeSellingEstimationDays(sellingEstimationDays) {
    cy.get(this.SELLING_ESTIMATION_DAYS_INPUT).type(sellingEstimationDays);
  }

  selectSettingType(settingType) {
    cy.get(this.SETTING_TYPE_INPUT + `[value='${settingType}']`).click();
  }

  clickAddSellingUom() {
    cy.get(this.ADD_SELLING_UOM_BUTTON).click();
  }

  selectSpecificUom(uom) {
    cy.get(this.UOM_CHECKBOX + `:contains('${uom}')`).click();
  }

  clickSetSellingPrice() {
    cy.get(this.SET_SELLING_PRICE_BUTTON).click({ force: true });
  }

  typeMinimumQuantity(minimumQuantity) {
    cy.get(this.MINIMUM_QUANTITY_INPUT).type(minimumQuantity);
  }

  typePrice(price) {
    cy.get(this.PRICE_INPUT).type(price);
  }

  typeMargin(margin) {
    cy.get(this.MARGIN_INPUT).type(margin);
  }

  clickSavePriceTier() {
    cy.get(this.SAVE_PRICE_TIER_BUTTON).click();
  }

  checkSellingPriceCalculation(beforeRounded, afterRounded) {
    cy.get(this.SELLING_PRICE_TEXT + `:contains('${beforeRounded}')`).should(
      "be.visible"
    );
    cy.get(
      this.ROUNDED_SELLING_PRICE_TEXT + `:contains('${afterRounded}')`
    ).should("be.visible");
  }

  clickNextToPreview() {
    cy.get(this.NEXT_TO_PREVIEW_BUTTON).click({ force: true });
  }

  clickSavePurchaseRequest() {
    cy.get(this.SAVE_PURCHASE_REQUEST_BUTTON).click();
  }

  clickConfirmOk() {
    cy.get(this.CONFIRM_OK_BUTTON).click();
  }
}

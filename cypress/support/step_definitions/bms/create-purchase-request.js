...
And("BMS - user search vendor code: {string}", (vendorId) => {
    ...
    cy
        .get(createPurchaseRequestPage.VENDOR_ID_INPUT)
        .invoke("val")
        .then(value => {
    ...
    cy
        .get(createPurchaseRequestPage.VENDOR_NAME_INPUT)
        .invoke("val")
        .should("not.be.empty")
        .then(value => {
    ...
    cy
        .get(createPurchaseRequestPage.VENDOR_ADDRESS_INPUT)
        .invoke("val")
        .should("not.be.empty")
        .then(value => {
    ...
    cy
        .get(createPurchaseRequestPage.VENDOR_TAX_TYPE_INPUT)
        .invoke("val")
        .should("not.be.empty")
        .then(value => {
...
And("BMS - user search buyer code: {string}", (buyerId) => {
    ...
    createPurchaseRequestPage.selectOption(0);
    cy
        .get(createPurchaseRequestPage.BUYER_ID_INPUT)
        .invoke("val")
        .then(value => {
    ...
    cy
        .get(createPurchaseRequestPage.BUYER_NAME_INPUT)
        .invoke("val")
        .should("not.be.empty")
        .then(value => {
    ...
    cy
        .get(createPurchaseRequestPage.BUYER_AREA_INPUT)
        .invoke("val")
        .should("not.be.empty")
        .then(value => {
...
And("BMS - user sets delivery request start date: {int} days from today", (int) => {
    createPurchaseRequestPage.setDeliveryRequestStartDate(Cypress.moment().add(int, 'days').format("ll"));
});

And("BMS - user sets delivery request end date: {int} days from today", (int) => {
    createPurchaseRequestPage.setDeliveryRequestEndDate(Cypress.moment().add(int, 'days').format("ll"));
});
        ...
        cy.get("@taxType").then(taxType => {
            if (taxType == "VAT"){
                ...
                cy.numberFormat(dpp).then(dpp => {
                    cy
                    .get(createPurchaseRequestPage.DPP_INPUT).eq(0)
                    .invoke("val")
                    .should("eq", dpp);
                ...
                cy.numberFormat(vat).then(vat => {
                    cy
                    .get(createPurchaseRequestPage.VAT_INPUT).eq(1)
                    .invoke("val")
                    .should("eq", vat);
    ...
    cy.get("@quantity").then(quantity => {
        cy.get("@rate").then(rate => {
            cy.get("@rateDiscount").then(rateDiscount => {
                cy.numberFormat(utils.totalAmount(quantity, rate, rateDiscount)).then(totalAmount => {
                    cy
                        .get(createPurchaseRequestPage.TOTAL_AMOUNT_INPUT)
                        .invoke("val")
                    ...
                    cy.get("@deliveryFeeDiscount").then(deliveryFeeDiscount => {
                        cy.get("@unloadingFee").then(unloadingFee => {
                            cy.get("@purchaseDiscount").then(purchaseDiscount => {
                                var minimumSellingPrice = utils.minimumSellingPrice(rate, rateDiscount, quantity, deliveryFee, deliveryFeeDiscount, unloadingFee, purchaseDiscount)
...
And("BMS - user add {int} purchase items with random product", (items) => {
    for (let i = 1; i <= items; i++) {
        ...
        var productName = productNameList[Math.floor(Math.random() * productNameList.length)]
        createPurchaseRequestPage.typeProduct(productName);
        ...
        cy.get(`@quantity ${i}`).then(quantity => {
            cy.get(`@rate ${i}`).then(rate => {
                cy.get(`@rateDiscount ${i}`).then(rateDiscount => {
                    cy.numberFormat(quantity * (rate - rateDiscount)).then(totalAmount => {
                        cy
                            .get(createPurchaseRequestPage.TOTAL_AMOUNT_INPUT)
                            .invoke("val")
                            .should("eq", totalAmount);
...
And("BMS - user fills selling estimation days: {int} digits random", (digits) => {
                ...
                cy.get("@margin").then(margin => {
                    ...
                    cy.numberFormat(price).then(price => {
                        cy.numberFormat(roundedPrice).then(roundedPrice => {
                            createPurchaseRequestPage.checkSellingPriceCalculation(price, roundedPrice);
                        });
                    });
                });
            });

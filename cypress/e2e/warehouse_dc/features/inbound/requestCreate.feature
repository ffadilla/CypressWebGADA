...
@focus 
  ...
  Scenario: User successfully create a new inbound Source/Request
    ...
    Examples:
      ...
      | "Pembelian" | "Toko DCFRS"  | "Warehouse"       | "Faris"             | "11"        | "22"         | "SELF PICKUP"  | "Indo"      | 20         |
@focus 
  Scenario: User successfully create a new inbound Source/Request (with global filter applied)
    ...
    And user applies "Warehouse DC FRS" and its store as global filters at inbound Request list
    And user clicks create new inbound request button
    Then user should be at inbound Request form
    And user should see "Warehouse DC FRS" applied as warehouse store dropdown on inbound Request form
    And user should see disabled global filter dropdown
